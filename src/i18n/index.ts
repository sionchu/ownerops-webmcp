...
const UI: Record<UiLocale, UiCopy> = {
  en: {
    ...
    scenario: { canonicalDisruption: "canonical disruption", markUnavailable: "Mark Minsoo unavailable", recoveryComparison: "recovery comparison", threeOptions: "Three response options", currentIncident: "Current incident", rankedBy: "Ranked by coverage, warnings, cost, and changes", recommended: "Recommended", addedPayroll: "Added payroll", weeklyHours: "Weekly hours", gap: "gap", warnings: "Warnings", changes: "Changes", preview: "Preview", previewing: "Previewing", coversShift: (name) => `${name} covers the shift`, takesShift: (name, start, end) => `${name} takes ${start}–${end} as a single reassignment.`, restoresCoverage: (ratio) => `Restores peak coverage with one schedule change; estimated labor ratio ${ratio}.` },
    metrics: { estimatedWeeklyLabor: "Estimated weekly labor", currentComparison: "in current comparison", fridayExpectedSales: "Friday expected sales", estimatedLaborRatio: "Estimated labor ratio", target: "target", coverageStatus: "status", peakCovered: "Peak covered", reviewFlag: "review flag" },
    preview: { humanEdit: "HUMAN EDIT", reviewed: "REVIEWED", agentProposal: "AGENT PROPOSAL", localImpactPending: "Local impact updated · agent review pending", reviewedReady: "Reviewed from current live state · ready to apply", candidateOnly: "Candidate only · committed schedule unchanged", payroll: "payroll", gap: "min gap", covered: "Peak covered", warning: "warning", reject: "Reject", applyReviewed: "Apply reviewed plan", reviewRequired: "Review required" },
  ...
  ko: {
    ...
    scenario: { canonicalDisruption: "기준 이슈", markUnavailable: "민수 결근 처리", recoveryComparison: "대응안 비교", threeOptions: "세 가지 대응 옵션", currentIncident: "현재 이슈", rankedBy: "커버리지, 경고, 비용, 변경 수 기준 정렬", recommended: "추천", addedPayroll: "추가 인건비", weeklyHours: "주간 근무시간", gap: "공백", warnings: "경고", changes: "변경", preview: "미리보기", previewing: "미리보는 중", coversShift: (name) => `${name}이(가) 근무를 대체`, takesShift: (name, start, end) => `${name}이(가) ${start}–${end} 근무를 한 번의 재배정으로 맡습니다.`, restoresCoverage: (ratio) => `한 번의 스케줄 변경으로 피크 커버리지를 복구합니다. 예상 인건비 비율 ${ratio}.` },
    metrics: { estimatedWeeklyLabor: "예상 주간 인건비", currentComparison: "현재 비교안 기준", fridayExpectedSales: "금요일 예상 매출", estimatedLaborRatio: "예상 인건비 비율", target: "목표", coverageStatus: "상태", peakCovered: "피크 커버 완료", reviewFlag: "검토 항목" },
    preview: { humanEdit: "사람이 수정", reviewed: "검토 완료", agentProposal: "에이전트 제안", localImpactPending: "영향 재계산 완료 · 에이전트 검토 대기", reviewedReady: "현재 실시간 상태 기준 검토 완료 · 적용 가능", candidateOnly: "후보만 표시 · 확정 스케줄은 변경되지 않음", payroll: "인건비", gap: "분 공백", covered: "피크 커버 완료", warning: "경고", reject: "거절", applyReviewed: "검토된 계획 적용", reviewRequired: "검토 필요" },
  ...
  ja: {
    ...
    scenario: { canonicalDisruption: "基準インシデント", markUnavailable: "Minsoo を欠勤にする", recoveryComparison: "復旧案の比較", threeOptions: "3つの対応案", currentIncident: "現在のインシデント", rankedBy: "カバレッジ、警告、コスト、変更数で順位付け", recommended: "推奨", addedPayroll: "追加人件費", weeklyHours: "週間時間", gap: "不足", warnings: "警告", changes: "変更", preview: "プレビュー", previewing: "プレビュー中", coversShift: (name) => `${name} がシフトをカバー`, takesShift: (name, start, end) => `${name} が ${start}–${end} を1回の再割当で担当します。`, restoresCoverage: (ratio) => `1回の変更でピークをカバー。推定人件費率 ${ratio}。` },
    metrics: { estimatedWeeklyLabor: "推定週間人件費", currentComparison: "現在の比較案", fridayExpectedSales: "金曜の予想売上", estimatedLaborRatio: "推定人件費率", target: "目標", coverageStatus: "ステータス", peakCovered: "ピークをカバー", reviewFlag: "レビュー項目" },
    preview: { humanEdit: "人が編集", reviewed: "レビュー済み", agentProposal: "エージェント提案", localImpactPending: "影響を更新 · エージェントレビュー待ち", reviewedReady: "現在のライブ状態からレビュー済み · 適用可能", candidateOnly: "候補のみ · 確定スケジュールは未変更", payroll: "人件費", gap: "分不足", covered: "ピークをカバー", warning: "警告", reject: "却下", applyReviewed: "レビュー済みプランを適用", reviewRequired: "レビュー必要" },
  ...
  es: {
    ...
    scenario: { canonicalDisruption: "incidencia canónica", markUnavailable: "Marcar a Minsoo no disponible", recoveryComparison: "comparación de recuperación", threeOptions: "Tres opciones de respuesta", currentIncident: "Incidencia actual", rankedBy: "Ordenadas por cobertura, alertas, coste y cambios", recommended: "Recomendada", addedPayroll: "Coste añadido", weeklyHours: "Horas semanales", gap: "hueco", warnings: "Alertas", changes: "Cambios", preview: "Vista previa", previewing: "En vista previa", coversShift: (name) => `${name} cubre el turno`, takesShift: (name, start, end) => `${name} cubre ${start}–${end} con una sola reasignación.`, restoresCoverage: (ratio) => `Recupera la cobertura punta con un cambio; ratio estimado ${ratio}.` },
    metrics: { estimatedWeeklyLabor: "Coste laboral semanal estimado", currentComparison: "en la comparación actual", fridayExpectedSales: "Ventas esperadas del viernes", estimatedLaborRatio: "Ratio laboral estimado", target: "objetivo", coverageStatus: "estado", peakCovered: "Pico cubierto", reviewFlag: "elemento a revisar" },
    preview: { humanEdit: "EDICIÓN HUMANA", reviewed: "REVISADO", agentProposal: "PROPUESTA DEL AGENTE", localImpactPending: "Impacto actualizado · revisión pendiente", reviewedReady: "Revisado desde el estado en vivo · listo para aplicar", candidateOnly: "Solo candidato · horario confirmado sin cambios", payroll: "coste", gap: "min sin cubrir", covered: "Pico cubierto", warning: "alerta", reject: "Descartar", applyReviewed: "Aplicar plan revisado", reviewRequired: "Revisión necesaria" },
  ...
  "zh-CN": {
    ...
    rail: { ownerOpsAgent: "OwnerOps 智能体", sharedStateActivity: "共享状态活动", checkingSharedState: "正在检查共享状态", liveSharedState: "实时共享状态", agentNotConnected: "智能体未连接", checkingBrowser: "正在检查浏览器能力", manualStillWorks: "仍可手动编辑排班", candidatePlan: "候选方案", humanEdit: "人工修改", reviewed: "已审核", agentProposal: "智能体建议", addedPayroll: "新增人工成本", weeklyHours: "每周工时", warnings: "警告", covered: "已覆盖", minutesGap: "分钟缺口", localImpactPending: "影响已更新 · 等待智能体审核", reviewedReady: "已按当前实时状态审核 · 可应用", previewUncommitted: "仅预览 · 已确认排班未更改", nextAgentAction: "下一步智能体操作", evaluateCurrentPlan: "审核当前方案", rereadExactEdit: "让智能体重新读取人工修改后的准确候选方案。", tryAsking: "试着让智能体处理", sameLiveSchedule: "结构化审核始终基于同一份实时排班。", laborRatio: "人工成本率" },
    ...
    scenario: { canonicalDisruption: "基准事件", markUnavailable: "将 Minsoo 标记为无法出勤", recoveryComparison: "恢复方案比较", threeOptions: "三种应对方案", currentIncident: "当前事件", rankedBy: "按覆盖、警告、成本和变更数量排序", recommended: "推荐", addedPayroll: "新增人工成本", weeklyHours: "每周工时", gap: "缺口", warnings: "警告", changes: "变更", preview: "预览", previewing: "正在预览", coversShift: (name) => `${name} 覆盖该班次`, takesShift: (name, start, end) => `${name} 通过一次重新分配承担 ${start}–${end}。`, restoresCoverage: (ratio) => `通过一次排班变更恢复高峰覆盖；预计人工成本率 ${ratio}。` },
    metrics: { estimatedWeeklyLabor: "预计每周人工成本", currentComparison: "当前比较方案", fridayExpectedSales: "周五预计销售额", estimatedLaborRatio: "预计人工成本率", target: "目标", coverageStatus: "状态", peakCovered: "高峰已覆盖", reviewFlag: "审核项" },
    preview: { humanEdit: "人工修改", reviewed: "已审核", agentProposal: "智能体建议", localImpactPending: "影响已更新 · 等待智能体审核", reviewedReady: "已按当前实时状态审核 · 可以应用", candidateOnly: "仅候选方案 · 已确认排班未更改", payroll: "人工成本", gap: "分钟缺口", covered: "高峰已覆盖", warning: "警告", reject: "拒绝", applyReviewed: "应用已审核方案", reviewRequired: "需要审核" },
