...
export function OwnerOpsApp() {
  ...
  const visibleImpact = previewImpact ?? impact;
  return (
    <main className={`app-shell ${hydrated ? "hydrated" : ""}`} data-industry={profile.id} style={{ "--oo-accent": profile.visual.accent, "--oo-accent-hover": profile.visual.accentHover, "--oo-accent-soft": profile.visual.accentSoft, "--oo-canvas": profile.visual.canvas, "--oo-surface": profile.visual.surface, "--oo-surface-elevated": profile.visual.surfaceElevated, "--oo-ink": profile.visual.ink, "--oo-secondary-ink": profile.visual.secondaryInk, "--oo-border": profile.visual.border, "--oo-focus-lane": profile.visual.focusLane, "--oo-agent-glow": profile.visual.agentGlow, "--oo-radius-card": profile.visual.radiusCard, "--oo-radius-shift": profile.visual.radiusShift, "--oo-motif-opacity": profile.visual.motifOpacity, "--oo-motion-theme": profile.visual.motionTheme } as CSSProperties}>
      <header className="topbar"><div className="brand"><span className="brand-mark">O</span><div><strong>OwnerOps</strong><span>{state.business.name}</span></div></div><div className="topbar-center"><span className="live-dot"/>Live shared state<span className="divider"/>{profile.label}</div><nav><button className="top-action" onClick={() => setSnapshotOpen(true)}><Icon name="copy"/>Snapshot</button><button className="top-action" onClick={() => runAction({ type: "reset_demo" })}><Icon name="refresh"/>Reset demo</button></nav><div className="context-stage"><span className="eyebrow">{profile.label} · Aug 24–30, 2026</span><h1>{profile.copy.headline}</h1><p>{profile.copy.scheduleContext}</p></div></header>
      <PreviewBar />
