...
export function OwnerOpsApp() {
  ...
  const visibleImpact = previewImpact ?? impact;
  return (
    <main className={`app-shell ${hydrated ? "hydrated" : ""}`} data-industry={profile.id} style={{ "--oo-accent": profile.visual.accent, "--oo-accent-hover": profile.visual.accentHover, "--oo-accent-soft": profile.visual.accentSoft, "--oo-surface-tint": profile.visual.surfaceTint, "--oo-canvas": profile.visual.canvas, "--oo-surface": profile.visual.surface, "--oo-ink": profile.visual.ink, "--oo-secondary-ink": profile.visual.secondaryInk, "--oo-radius": profile.visual.radiusStyle } as CSSProperties}>
