...
### AC11 — recipe / theoretical usage
For food profiles, menu item sales can derive theoretical ingredient usage from recipe lines. If actual count/usage differs, OwnerOps can surface the variance without pretending to know an unsupported cause.
...
One plan may contain staffing + purchase + prep + task changes. The UI visibly shows candidate vs committed state.
...
A consequential plan cannot be applied until reviewed and matching current preview id/version. Applying commits the supported changes and clears the preview.
...
External reference/weather provider failure does not break the product. The relevant observation is marked stale/seed and deterministic fallback remains available.

### AC22 — no hidden second state
Equivalent UI and WebMCP actions produce equivalent canonical StoreState. No agent-only duplicate business model exists.
...
9. Ask “원두 비싸게 사고 있어?” and inspect actual-vs-reference provenance.
...
The repository must continue exposing runnable commands for:
- test;
- lint;
- typecheck;
- production build;
...
Do not declare this RE0 complete while old staffing-only acceptance tests or docs contradict the new StoreState behavior.
